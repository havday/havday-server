= API 문서
:doctype: book
:icons: font
:source-highlighter: highlightjs
:toc: left
:toclevels: 3

== 유저를 가입시킨다.

=== Request

include::{snippets}/register-user/success/http-request.adoc[]

=== Request Fields

include::{snippets}/register-user/success/request-fields.adoc[]

==== Response

include::{snippets}/register-user/success/http-response.adoc[]

== 주소를 추가한다.

=== Request

include::{snippets}/save-address/success/http-request.adoc[]

=== Request Fields

include::{snippets}/save-address/success/request-fields.adoc[]

==== Response

include::{snippets}/save-address/success/http-response.adoc[]